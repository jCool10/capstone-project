...
from src.node.base_node import BaseNode, NodeWithScore
from src.utils.utils import globals_helper
...
class BM25Retriever(BaseRetriever):
    ...
    def from_defaults(
        ...
        # ensure only one of nodes, or docstore is passed
        if sum(bool(val) for val in [nodes, docstore]) != 1:
        ...
        assert (
            nodes is not None
        ), "Please pass exactly one of nodes, or docstore."
