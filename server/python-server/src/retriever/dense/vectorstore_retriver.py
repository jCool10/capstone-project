...
from src.constants import DEFAULT_SIMILARITY_TOP_K
from src.retriever.base_retriver import BaseRetriever, QueryBundle
from src.node.base_node import NodeWithScore, ObjectType
...
class VectorIndexRetriever(BaseRetriever):
    ...
    def _build_node_list_from_query_result(
        ...
        if query_result.nodes is None:
            ...
            print("Not supported for vector store that not keep text. Return empty list!!!")
            return []
        else:
            ...
            for i in range(len(query_result.nodes)):
                ...
                if (not self._vector_store.stores_text) or (
                    ...
                    if self._storage_context.docstore.document_exists(node_id):
                        query_result.nodes[
                            i
                        ] = self._storage_context.docstore.get_node(  # type: ignore[index]
                            node_id
                        )
