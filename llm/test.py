...
from src.dataloader import QADataloader
...
def parse_arguments():
    ...
    model_group.add_argument(
        "--model_name_or_path",
        type=str,
        default="google/umt5-small",
        help="Model name or path",
    )
    model_group.add_argument(
        "--model_type", type=str, default="SEQ_2_SEQ_LM", help="Type of model to train"
    )
    model_group.add_argument(
        "--model_dtype", type=str, default="auto", help="Model torch_dtype"
    )
    model_group.add_argument(
        "--print_model_key",
        action="store_true",
        help="Whether to print out model structure",
    )
    model_group.add_argument(
        "--shard_model",
        action="store_true",
        help="Sharded the model weight to fit on memory",
    )
    model_group.add_argument(
        "--shard_model_merge",
        action="store_true",
        help="Shard the model to load and then merge weight to peft",
    )
    model_group.add_argument(
        "--max_model_shard_size",
        type=str,
        default="1GB",
        help="Max size per model shard",
    )
    model_group.add_argument(
        "--use_flash_attention_2",
        action="store_true",
        help="Enable flash attention 2 or not"
        "More info: https://huggingface.co/docs/transformers/perf_infer_gpu_one",
    )
    model_group.add_argument(
        "--better_transformer", action="store_true", help="Enable flash attention"
    )
    model_group.add_argument(
    ...
    peft_group.add_argument(
        "--lora_alpha", type=int, default=64, help="Alpha parameter for LoRA scaling"
    )
    peft_group.add_argument(
        "--lora_dropout",
        type=float,
    ...
    peft_group.add_argument(
        ...
        type=str,
        default=None,
        help="List of modules apart from "
    ...
    bitsandbytes_group.add_argument(
        "--use_4bit",
        action="store_true",
    ...
    bitsandbytes_group.add_argument(
        "--use_8bit",
        action="store_true",
        help="Activate 8-bit precision base model loading",
    )
    ...
    training_group.add_argument(
        "--log_weights_cpkt",
        action="store_true",
        help="Whether to log checkpoint to wandb",
    )
    training_group.add_argument(
        "--with_tracking",
        action="store_true",
    ...
    training_group.add_argument(
        "--optim_name",
        type=str,
        default="PagedLion8bit",
        help="Name of optimizer in bnb lib",
    )
    training_group.add_argument(
        "--weight_decay", type=float, default=0.2, help="Weight decay"
    )
    ...
    training_group.add_argument(
        "--warmup_steps", type=int, default=20, help="Num warmup steps"
    )
    training_group.add_argument(
    ...
    training_group.add_argument(
        "--do_test", action="store_true", help="Flag to perform testing"
    )
    training_group.add_argument(
        "--do_eval", action="store_true", help="Flag to perform evaluation"
    )

    training_group.add_argument(
        "--merge_weight_eval",
        action="store_true",
        help="Flag to enable merge weight from peft for faster eval",
    )
    training_group.add_argument(
    ...
    training_group.add_argument(
        "--enable_model_offload", action="store_true", help="Enable model offload"
    )
    training_group.add_argument(
        "--minimum_free_spaces",
        type=int,
        default=1,
        help="Minimum free spaces to keep in GB",
    )
    training_group.add_argument(
        "--llm_int8_enable_fp32_cpu_offload", action="store_true", help=""
    )
    training_group.add_argument(
    ...
    training_group.add_argument(
        "--convert_cpkt",
        action="store_true",
        help="Convert checkpoint into model and push to" "hub",
    )

    dataloader_group = parser.add_argument_group("Dataloader Arguments")
    dataloader_group.add_argument(
        "--dataset_name", type=str, default="Instruction_en-vn_mix", help="Dataset name"
    )
    dataloader_group.add_argument(
    ...
    dataloader_group.add_argument(
        "--generative_eval_batch_size",
        type=int,
        default=8,
        help="Generative evaluation batch size",
    )
    dataloader_group.add_argument(
        "--text_column", type=str, default="prompt", help="Text column"
    )
    dataloader_group.add_argument(
        "--label_column", type=str, default="target", help="Label column"
    )
    dataloader_group.add_argument(
        "--response_template",
        type=str,
    ...
    dataloader_group.add_argument(
        "--block_size", type=int, default=768, help="Block size for group text function"
    )
    dataloader_group.add_argument(
        "--do_group_texts",
        action="store_true",
        help="Do group text, great for pretraining phase",
    )
    dataloader_group.add_argument(
        "--model_max_length", type=int, default=1024, help="The model maximum length"
    )
    dataloader_group.add_argument(
        "--context_length",
        type=int,
        default=768,
        help="The model maximum context length",
    )
    dataloader_group.add_argument(
        "--train_file", nargs="+", type=str, default=None, help="List of training files"
    )
    dataloader_group.add_argument(
        "--each_train_file_percentage",
        nargs="+",
        type=int,
        default=None,
    ...
    dataloader_group.add_argument(
        "--val_file", nargs="+", type=str, default=None, help="List of validation files"
    )

    dataloader_group.add_argument(
        "--test_file", nargs="+", type=str, default=None, help="List of test files"
    )
    ...
    dataloader_group.add_argument(
        "--config_type",
        type=str,
        default="AdvanceInstructSample",
        help="Configuration type",
    )
    dataloader_group.add_argument(
        "--no_preprocess_data",
        action="store_true",
        help="Whether to tokenized the data first"
        "turn off this flag for large dataset",
    )
    dataloader_group.add_argument(
        "--do_perplexity_eval",
        action="store_true",
        help="Flag to enable perplexity computation, relevant when using casual-LM",
    )
    dataloader_group.add_argument(
        "--max_eval_generative_samples",
        type=int,
        default=50,
        help="Max generative examplew for manual evaluation",
    )
    dataloader_group.add_argument(
        "--do_generative_eval",
        action="store_true",
        help="Flag to enable model.generate eval",
    )
    dataloader_group.add_argument(
        "--max_eval_perplexity_samples",
        type=int,
        default=50,
        help="Max evaluation examples for perplexity evaluation",
    )
    ...
    generation_group.add_argument(
        "--top_p", type=float, default=1.0, help="Top-p value"
    )
    generation_group.add_argument(
        "--do_sample", action="store_true", help="Enable sampling (default: True)"
    )
    generation_group.add_argument(
        "--no_repeat_ngram_size",
        type=int,
        default=0,
        help="No repeat n-gram size (default: 3)",
    )
    generation_group.add_argument(
        "--num_beams", type=int, default=1, help="Number of beams (default: 5)"
    )
    generation_group.add_argument(
        "--early_stopping",
        action="store_true",
    ...
    generation_group.add_argument(
        "--penalty_alpha", type=float, default=None, help="Penalty alpha (default: 1.2)"
    )
    generation_group.add_argument(
        "--repetition_penalty",
        type=float,
        default=1.0,
        help="Repetition penalty (default: 2.5)",
    )
    generation_group.add_argument(
        "--temperature", type=float, default=1.0, help="Temperature (default: 1.5)"
    )
    generation_group.add_argument(
        "--no_truncation", action="store_true", help="Enable truncation (default: True)"
    )
    generation_group.add_argument(
        "--encoder_repetition_penalty",
        type=float,
        default=1.0,
        help="Encoder repetition penalty (default: 2.0)",
    )
    generation_group.add_argument(
        "--max_length", type=int, default=1024, help="Max length (default: 1024)"
    )
    generation_group.add_argument(
        "--max_new_tokens", type=int, default=None, help="Max new tokens "
    )
    generation_group.add_argument(
        "--deep_speed_inf", action="store_true", help="Enable deep speed inference"
    )
    generation_group.add_argument(
        "--use_default_gen_config",
        action="store_true",
        help="Use model's default gen config",
    )
    generation_group.add_argument(
        "--injection_policy",
        type=json.loads,
    ...
    if args.each_train_file_percentage:
        assert len(args.each_train_file_percentage) == len(args.train_file), (
            "The each_train_file_percentage length must be "
            "equal to the numbers of files in the train_file"
        )
        assert sum(args.each_train_file_percentage) == 100, (
            "The each_train_file_percentage arguments must be a list of int"
            "that add up to 100%"
        )
    ...
    if args.deep_speed_inf and not args.merge_weight_eval:
        warnings.warn(
            "Deepspeed inference is enable but merge weight eval is disable."
            "This might slow down the generation a bit."
        )

    if not args.deep_speed_inf:
        warnings.warn(
            "Deepspeed inference is disable, this might result in very slow inference"
        )
    ...
    if args.injection_policy:
        ...
        try:
            ...
            for idx, module in enumerate(
                list(args.injection_policy.values())[0].split(".")
            ):
                if idx == 0:
                    fetch_policy = module_inject
                fetch_policy = getattr(fetch_policy, module)
        except Exception as e:
            warnings.warn(
                "Invalid injection policy for deepspeed, assuming the values is user specified"
                f"Error message: {e}"
            )
    ...
    if args.injection_policy and args.auto_kernel_injection:
        warnings.warn(
            "Cannot use both user specified injection policy and kernel injection"
    ...
    if not args.auto_kernel_injection and args.injection_policy and args.deep_speed_inf:
        warnings.warn(
            f"Be sure to use auto_kernel_injection first to check"
