...
def poor_man_llm_load(
    ...
    try:
        ...
        if "offload_folder" in additional_kwargs.keys():
            (
                os.makedirs(additional_kwargs["offload_folder"])
                if not os.path.exists(additional_kwargs["offload_folder"])
                else None
            )
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if model_type == "CAUSAL_LM":
                sharded_model = AutoModelForCausalLM.from_pretrained(
                    tmp_dir, **additional_kwargs
                )
            else:
                sharded_model = AutoModelForSeq2SeqLM.from_pretrained(
                    tmp_dir, **additional_kwargs
                )
            return sharded_model
    except Exception as e:
        raise f"Unable to offload {model_name} with the " f"following error: {e}"
