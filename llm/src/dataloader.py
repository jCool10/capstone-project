...
from src.utils import dist_print, in_notebook, AdvanceQAExample, AdvanceInstructSample
...
class AdvanceQa(Dataset):
    ...
    def __init__(
        ...
        if not percentage_weights:
            percentage_weights = [
                math.floor(100 / len(json_file_paths))
                for _ in range(len(json_file_paths))
            ]
        ...
        for json_path, percentage_weight in tzip(
            json_file_paths,
            percentage_weights,
            ...
            num_examples_each_file = math.floor(
                num_examples * (percentage_weight / 100)
            )
            loading_bar_desc = (
                f"Loading data from {os.path.basename(json_path)} for split {split}"
            )
            loading_bar = tqdm(
                total=num_examples_each_file,
                colour="green",
            ...
            try:
                ...
                dist_print(
                    f"Loading {num_examples_each_file} examples with percentage of {percentage_weight} from {file_name}..."
                )
                iterable_json_data = load_dataset(
                    extension,
                ...
                for idx, data in enumerate(iter(iterable_json_data["train"])):
                    ...
                    if get_example:
                        try:
                            config_data = self.config_type(**data).get_example(
                            ...
                            if task_type == "CAUSAL_LM":
                                if split == "train" or do_generative_eval:
                                    config_data_tokenzied = tokenizer(
                                        config_data["prompt"]
                                    )
                                    if (
                                        len(config_data_tokenzied["input_ids"])
                                        > max_seq_length
                                    ):
                                        total_skipped += 1
                            ...
                            raise f"Missing keys to fill for {config_data} in item {idx} in {file_name}" f"Error message: {e}"
                    else:
                        config_data = data
...
class QADataloader:
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            ...
            padding_side=(
                "left"
                if task_type == "CAUSAL_LM" and "gpt2" not in model_name
                else "right"
            ),
        )
        ...
        for key, value in DEFAULT_TOKENS.items():
            if not getattr(self.tokenizer, key, None):
                print(
                    f" {model_name}'s tokenizer does not have {key} token, setting it to {value}\n"
                )
        ...
        if not max_eval_generative_samples:
            ...
            assert max_eval_samples >= max_eval_generative_samples, (
                "Max eval generative samples can't be larger than the"
                "whole eval dataset"
            )
        ...
        if not max_eval_perplexity_samples:
            ...
            assert max_eval_samples >= max_eval_perplexity_samples, (
                "Max eval perplexity samples can't be larger than the"
                "whole eval dataset"
            )
    ...
    def __call__(self, *args, **kwargs) -> Union[Set[DataLoader], Set]:
        ...
        if self.train_file is not None:
            ...
            train_dataset = self.load_data(
                self.train_file, self.max_train_samples, split="train"
            )
            dataloaders["train"] = self.get_dataloader(
                (
                    train_dataset
                    if self.no_preprocess_data
                    else self.preprocess_data(train_dataset, split="train")
                ),
                shuffle_flag=True,
        ...
        if self.val_file is not None:
            ...
            if self.do_generative_eval or self.task_type == "SEQ_2_SEQ_LM":
                ...
                eval_dataset_input = (
                    eval_dataset_input
                    if self.no_preprocess_data
                    else self.preprocess_data(eval_dataset_input)
                )
                dataloaders["eval"]["generative_eval"] = self.get_dataloader(
                    eval_dataset_input, batch_size=self.generative_eval_batch_size
                )
            if self.do_perplexity_eval and not self.task_type == "SEQ_2_SEQ_LM":
                ...
                eval_dataset_input = (
                    ...
                    if self.no_preprocess_data
                        ...
                        eval_dataset_input, perplexity_eval=self.do_perplexity_eval
                    )
                )
                dataloaders["eval"]["perplexity_eval"] = self.get_dataloader(
                    eval_dataset_input, batch_size=self.perplexity_eval_batch_size
                )
        ...
        if self.test_file is not None:
            ...
            if self.do_generative_eval or self.task_type == "SEQ_2_SEQ_LM":
                dataloaders["test"]["generative_eval"] = self.get_dataloader(
                    (
                        test_dataset
                        if self.no_preprocess_data
                        else self.preprocess_data(test_dataset)
                    ),
                    batch_size=self.test_batch_size,
                )
            if self.do_perplexity_eval and not self.task_type == "SEQ_2_SEQ_LM":
                dataloaders["test"]["perplexity_eval"] = self.get_dataloader(
                    (
                        test_dataset
                        if self.no_preprocess_data
                        else self.preprocess_data(
                            test_dataset, perplexity_eval=self.do_perplexity_eval
                        )
                    ),
                    batch_size=self.test_batch_size,
                )
    ...
    def export_dataloaders_to_json(self, dataloaders_info):
        # Save the dataloaders info to a JSON file
        with open("dataloaders_info.json", "w") as json_file:
            json.dump(dataloaders_info, json_file, indent=4)
        print("Dataloaders information exported to dataloaders_info.json")

    def load_data(
        self,
        data_files: List[str],
        num_example: int = 100000,
        split: str = "train",
        get_example: bool = True,
        do_perplexity_eval: bool = False,
        do_generative_eval: bool = False,
        ...
        dataset = AdvanceQa(
            ...
            percentage_weights=(
                self.each_train_file_percentage if split == "train" else None
            ),
    ...
    def preprocess_data(self, dataset, split=None, perplexity_eval: bool = False):
        tokenized_dataset = list(
            map(
                lambda data: self.tokenize_function(data, split, perplexity_eval),
                dataset,
            )
        )
    ...
    def tokenize_function(
        self, data: hfDataset, split: str = None, perplexity_eval: bool = False
    ):
        ...
        inp_tokens = self.tokenizer(
            ...
            max_length=(
                ...
                if split == "train" or perplexity_eval
        ...
        if self.task_type == "SEQ_2_SEQ_LM":
            ...
            tgt_tokens = self.tokenizer(
                ...
                return_special_tokens_mask=True,
                max_length=(
                    self.model_max_length if split == "train" else self.context_length
                ),
            )
            target_ids = tgt_tokens["input_ids"]
            target_mask = tgt_tokens["attention_mask"].bool()
    ...
    def get_dataloader(
        self, dataset, shuffle_flag: bool = False, batch_size: int = 1
    ) -> DataLoader:
        """
        ...
        """
        sampler = (
            RandomSampler(data_source=dataset, generator=self.generator)
            if shuffle_flag
        ...
        if self.no_preprocess_data:
            ...
            collate_function = DataCollatorForCompletionOnlyLM(
