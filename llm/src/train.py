...
from src.utils import in_notebook, poor_man_llm_load
...
def merge_adapter(
    ...
    max_memory: dict = {0: "0.3GB"},
    max_shard_size: str = "500MB",
    no_split_module_classes: List[str] = None,
    accelerator=None,
):
    peft_adapter.save_pretrained(
        adapter_save_path,
    ...
    if model_type == "CAUSAL_LM":
        if not shard_model:
            base_model = AutoModelForCausalLM.from_pretrained(
                base_model_name, **offload_config
            )
        else:
            base_model = poor_man_llm_load(
                base_model_name,
    ...
    if getattr(base_model, "quantization_method", None) == "gptq":
        warnings.warn(
            f"The model {base_model_name} is gptq quantized and cannot be merged to LORA layers.\n"
    ...
    base_model = dispatch_model(
        base_model, device_map=device_map, offload_dir="offload_inf"
    )
...
def prepare_any(prepare_dict: dict, distributed_type, accelerator):
    def get_grouped_parameters(adapter, weight_decay):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p for n, p in adapter.named_parameters() if n in decay_parameters
                ],
    ...
    if distributed_type != DistributedType.DEEPSPEED:
        ...
        optimizer_grouped_parameters = get_grouped_parameters(
            prepare_dict["adapter"], prepare_dict["weight_decay"]
        )
        optimizer = getattr(bnb.optim, prepare_dict["optim_name"])(
            optimizer_grouped_parameters, lr=prepare_dict["lr"]
        )
        accelerator.print(
        ...
        if "scheduler" in accelerator.state.deepspeed_plugin.deepspeed_config:
            ...
            adapter, dataloaders["train_dataloader"], optimizer, lr_scheduler = (
                accelerator.prepare(
                    prepare_dict["adapter"],
                    prepare_dict["train_dataloader"],
                    optimizer,
                    lr_scheduler,
                )
            )
    ...
    if "perplexity_eval_dataloader" in prepare_dict:
        dataloaders["perplexity_eval_dataloader"] = accelerator.prepare(
            prepare_dict["perplexity_eval_dataloader"]
        )
    if "generative_eval_dataloader" in prepare_dict:
        dataloaders["generative_eval_dataloader"] = accelerator.prepare(
            prepare_dict["generative_eval_dataloader"]
        )
    if "test_dataloader" in prepare_dict:
        dataloaders["test_dataloader"] = accelerator.prepare(
            prepare_dict["test_dataloader"]
        )
...
def train(training_args, qa_dataloader, qa_dataloader_instance):
    ...
    if not use_default_gen_config:
        try:
            ...
            if len(unused_config) > 0:
                accelerator.print(f"Unused config: {unused_config}")
        except Exception as e:
            warnings.warn(
                f"The model {model_name_or_path} does not have a generation config"
            )
    ...
    accelerator.print(
        " Print out a couple samples for tokenizer compatibility check for multilingual task\n"
    )
    ...
    for idx, data in enumerate(iter(qa_dataloader_instance["test"]["perplexity_eval"])):
        ...
        accelerator.print(
            "\n Input: "
            + qa_dataloader.tokenizer.decode(
                data["input_ids"][0], skip_special_tokens=False
            )
        )
        ...
        accelerator.print(
            "\n Label:"
            + qa_dataloader.tokenizer.decode(labels[0], skip_special_tokens=False)
        )
        accelerator.print(
            "\n==============================================================================\n"
        )
    ...
    max_memory = (
        f"{int(torch.cuda.mem_get_info()[0] / 1024 ** 3) - minimum_free_spaces}GB"
    )
    n_gpus = torch.cuda.device_count()
    max_memory = {i: max_memory for i in range(n_gpus)}

    accelerator.print(
        f"System max memory: {max_memory}\n"
    ...
    if use_flash_attention_2:
        full_model_config["use_flash_attention_2"] = True
    ...
    if model_offload:
        full_model_config = {**full_model_config, **offload_config}

    # creating model
    if task_type == "CAUSAL_LM":
        if not shard_model:
            base_model = AutoModelForCausalLM.from_pretrained(
                model_name_or_path, **full_model_config
            )
        else:
            base_model = poor_man_llm_load(
                model_name_or_path,
    ...
    if (
        use_4bit
        or use_8bit
        or getattr(base_model, "quantization_method", None) == "gptq"
    ):
        accelerator.print(f"Preparation for kbit training...")
        base_model = prepare_model_for_kbit_training(
            base_model, use_gradient_checkpointing=gradient_checkpointing
        )  # Prepare model in peft already include gradient-checkpoint, freeze params
    elif gradient_checkpointing:
        base_model.gradient_checkpointing_enable()
    else:
        warnings.warn(
            "You disable gradient checkpoint, this will result in vram consumtion"
        )
    ...
    adapter = get_peft_model(
        base_model, peft_config=peft_config, adapter_name=dataset_name
    )
    if gradient_checkpointing:
        adapter.gradient_checkpointing_enable()  # Double check!
    ...
    num_update_steps_per_epoch = math.ceil(
        len(qa_dataloader_instance["train"]) / gradient_accumulation_steps
    )
    ...
    if do_eval:
        if perplexity_eval:
            prepare_dict["perplexity_eval_dataloader"] = qa_dataloader_instance["eval"][
                "perplexity_eval"
            ]
        if generative_eval:
            prepare_dict["generative_eval_dataloader"] = qa_dataloader_instance["eval"][
                "generative_eval"
            ]
    else:
    ...
    adapter, optimizer, dataloaders, lr_scheduler = prepare_any(
        prepare_dict, accelerator.distributed_type, accelerator
    )
    ...
    if with_tracking:
        ...
        accelerator.init_trackers(
            ...
            init_kwargs={
                "wandb": {
                    "name": f"{dataset_name}_completed_step{resume_step if resume_step is not None else 0}"
                }
            },
        )
    ...
    def save_push():
        ...
        unwrapped_adapter.save_pretrained(
            dataset_name,
            is_main_process=accelerator.is_main_process,
            save_function=accelerator.save,
    ...
    def save_state():
        ...
        if accelerator.is_main_process and training_args.log_weights_cpkt:
            ...
            accelerator.get_tracker(name="wandb", unwrap=True).log_artifact(
                wandb_artifact
            )
    ...
    progress_bar_epoch = tqdm(
        total=num_epochs - starting_epoch,
        desc=f"Training progress on process {accelerator.process_index}",
    ...
    for epoch in range(starting_epoch, num_epochs):
        with TorchTracemalloc() as tracemalloc:
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                if accelerator.sync_gradients:
                    ...
                    remaining = (
                        (progress_bar_step.total - progress_bar_step.n) / rate
                        if rate and progress_bar_step.total
                        else 0
                    )
                        ...
                        accelerator.log(
                            {
                                ...
                                "Elapsed(hours)": progress_bar_step.format_dict[
                                    "elapsed"
                                ]
                                / 60
                                / 60,
        ...
        if do_eval:
            cur_time = "_".join(
                str(datetime.datetime.now().strftime("%Y-%m-%d %H-%M-%S")).split()
            )
            if merge_weight_eval:
                ...
                warnings.warn(
                    f"Weight from peft not merged yet, this may result in slower inference"
                )
                accelerator.wait_for_everyone()
            ...
            if deep_speed_inf:
                ...
                injection_config = (
                    ...
                    if auto_kernel_injection or injection_policy
                    else {}
                )

                inference_model = deepspeed.init_inference(
                    inference_model, mp_size=world_size, **injection_config
                )
            ...
            if task_type == "SEQ_2_SEQ_LM" and generative_eval:
                ...
                if generative_eval:
                    with TorchTracemalloc() as tracemalloc:
                        with torch.no_grad():
                            for idx, batch in enumerate(
                                ...
                                preds = (
                                    accelerator.gather_for_metrics(outputs)
                                    .detach()
                                    .cpu()
                                    .numpy()
                                )
                    ...
                    # Printing the GPU memory usage details such as allocated memory, peak memory, and total memory usage
                    accelerator.print(
                        "GPU Memory before entering the eval : {}".format(
                ...
                try:
                    ...
                    log_path = os.path.join(
                        cur_dir,
                        f"src/models/runs/logs/log_dir_e{epoch}_{model_name}_{cur_time}.txt",
                    )
                    with open(log_path, "w") as log_file:
                        ...
                        for i in range(0, 10):
                            ...
                            accelerator.print(
                                f"        Question: {qa_dataloader.dataset['eval'][idx][text_column]}\n"
                            )
                            accelerator.print(
                                f"    Evaluation prediction: {eval_preds[idx]}\n"
                            )
                            accelerator.print(
                                f"    Actual label: {qa_dataloader.dataset['eval'][idx][label_column]}\n"
                            )
                    ...
                    warnings.warn(
                        f"Can't save config for this run {epoch}\n"
                        f"Error message: {e}"
                    )
                    pass
                                    ...
                                    for idx, batch in enumerate(
                                        ...
                                        with torch.no_grad():
                                            outputs = inference_model.generate(
                                                **batch,
                                                generation_config=generation_config,
                                                synced_gpus=accelerator.distributed_type
                                                != DistributedType.NO,
                                                pad_token_id=tokenizer.pad_token_id,
                                            )  # synced_gpus=True for Distributed training
                                        outputs = accelerator.pad_across_processes(
                    ...
                    for step, batch in enumerate(
                        ...
                        losses.append(
                            accelerator.gather_for_metrics(
                                loss.repeat(qa_dataloader.perplexity_eval_batch_size)
                            )
                        )
                ...
                if generative_eval:
                    try:
                        ...
                        with open(log_path, "w") as log_file:
                            ...
                            for idx in range(0, len(eval_preds) - 1):
                                try:
                                    ...
                                    warnings.warn(
                                        f"Can't write config for prediction with idx {idx}\n"
                                        f"Error message: {e}"
                                    )
                                    pass
                            log_file.write(f"\n     Training arguments: \n")
                        ...
                        warnings.warn(
                            f"Can't save config for this epoch {epoch}\n"
                            f"Error message: {e}"
                        )
