...
class RAGEvaluator:
    ...
    def __init__(self, ground_truth_file: str = "data/ground_truth.json"):
        ...
        logger.info(f"RAG Evaluator initialized with {len(self.ground_truth['test_cases'])} test cases")
    ...
    def create_chunk_to_doc_mapping(self, collection_name: str):
        ...
        try:
            ...
            logger.info("Chunk to document mapping will be built dynamically during evaluation")
    ...
    def extract_document_id_from_chunk(self, chunk_result: Dict) -> int:
        ...
        doc_keywords = {
            1: ["Trí tuệ nhân tạo", "Machine Learning", "AI", "artificial intelligence", "deep learning", "neural", "supervised learning", "unsupervised learning", "reinforcement learning"],
            2: ["Blockchain", "cryptocurrency", "Bitcoin", "Ethereum", "Satoshi Nakamoto", "smart contract", "DApps", "distributed ledger", "hash"],
            3: ["Khoa học dữ liệu", "Data Science", "Python", "R", "Pandas", "NumPy", "Scikit-learn", "Matplotlib", "Big Data", "Hadoop", "Spark"],
            4: ["Cybersecurity", "An ninh mạng", "phishing", "malware", "ransomware", "firewall", "encryption", "two-factor", "penetration testing"],
            5: ["Cloud Computing", "DevOps", "AWS", "Azure", "Docker", "Kubernetes", "CI/CD", "containerization", "IaaS", "PaaS", "SaaS"],
            6: ["Internet of Things", "IoT", "Smart Cities", "sensors", "Edge computing", "smart home", "Industrial IoT", "IIoT"],
            7: ["Mobile Development", "App Design", "iOS", "Android", "React Native", "Flutter", "Xamarin", "UX", "UI", "PWA", "ASO"],
            8: ["Web Development", "Frontend", "HTML", "CSS", "JavaScript", "React", "Vue.js", "Angular", "Node.js", "SPA", "CDN"],
            9: ["Database", "Data Management", "MySQL", "PostgreSQL", "NoSQL", "MongoDB", "Cassandra", "Redis", "SQL", "ETL"],
            10: ["Software Engineering", "Agile", "SDLC", "Scrum", "TDD", "design patterns", "documentation", "maintainable"]
        }
    ...
    def calculate_precision_at_k(self, query_results: List[Dict], relevant_docs: List[int], k: int) -> float:
        """
    ...
    def calculate_recall_at_k(self, query_results: List[Dict], relevant_docs: List[int], k: int) -> float:
        """
        Calculate Recall@k
    ...
    def calculate_mrr(self, query_results: List[Dict], relevant_docs: List[int]) -> float:
        """
    ...
    def calculate_dcg_at_k(self, query_results: List[Dict], relevance_scores: Dict[int, int], k: int) -> float:
        """
    ...
    def calculate_ideal_dcg_at_k(self, relevance_scores: Dict[int, int], k: int) -> float:
        """
    ...
    def calculate_ndcg_at_k(self, query_results: List[Dict], relevance_scores: Dict[int, int], k: int) -> float:
        """
    ...
    def create_relevance_scores(self, test_case: Dict) -> Dict[int, int]:
        ...
        """
        relevance_levels = self.ground_truth.get("relevance_levels", {
            "highly_relevant": 3,
            "relevant": 2,
    ...
    def evaluate_single_query(self, test_case: Dict, collection_name: str, search_method: str = "dense") -> Dict[str, Any]:
        """
        ...
        for k in k_values:
            result["precision_at_k"][f"@{k}"] = self.calculate_precision_at_k(query_results, relevant_docs, k)
            result["recall_at_k"][f"@{k}"] = self.calculate_recall_at_k(query_results, relevant_docs, k)
            result["ndcg_at_k"][f"@{k}"] = self.calculate_ndcg_at_k(query_results, relevance_scores, k)
    ...
    def _query_sparse_only(self, query: str, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.rag_service, 'hybrid_engine') and self.rag_service.hybrid_engine:
                logger.info("Hybrid engine found, attempting BM25 search...")
                
                try:
                    # IMPORTANT: Load BM25 index first before using _get_sparse_results
                    bm25_index = self.rag_service.hybrid_engine.get_or_create_bm25_index(collection_name)
                    if bm25_index and bm25_index.bm25:
                        logger.info(f"BM25 index loaded successfully for {collection_name}")
                        
                        bm25_results = self.rag_service.hybrid_engine._get_sparse_results(
                            query, collection_name, k=10
                        )
                        
                        if bm25_results:
                            logger.info(f"Hybrid engine BM25 returned {len(bm25_results)} results")
                            return {
                                "success": True,
                                "data": {
                                    "docs": bm25_results,
                        ...
                        logger.warning(f"BM25 index not available or not built for {collection_name}")
                except Exception as e:
            ...
            if hasattr(self.rag_service, 'hybrid_engine') and self.rag_service.hybrid_engine:
                hybrid_bm25_indices = getattr(self.rag_service.hybrid_engine, 'bm25_indices', {})
                logger.debug(f"Hybrid engine BM25 indices: {list(hybrid_bm25_indices.keys())}")
                ...
                if hybrid_bm25_index:
                    ...
                    try:
                        ...
                        logger.info(f"Hybrid engine BM25 index search returned {len(results)} results")
            ...
            # No BM25 available
            logger.error("BM25 index not available for sparse search through any method")
            return {
                "success": False,
    ...
    def _empty_result(self, query_id: str, query: str, search_method: str) -> Dict[str, Any]:
    ...
    def evaluate_all_queries(self, collection_name: str, search_method: str = "dense") -> List[Dict[str, Any]]:
        """
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            result = self.evaluate_single_query(test_case, collection_name, search_method)
            results.append(result)
            
        return results
        
    def calculate_average_metrics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for k in k_values:
            key = f"@{k}"
            summary["avg_precision_at_k"][key] = np.mean(avg_precision[key]) if avg_precision[key] else 0.0
            summary["avg_recall_at_k"][key] = np.mean(avg_recall[key]) if avg_recall[key] else 0.0
            summary["avg_ndcg_at_k"][key] = np.mean(avg_ndcg[key]) if avg_ndcg[key] else 0.0
            
        return summary
        
    def compare_three_methods(self, dense_results: Dict, sparse_results: Dict, hybrid_results: Dict) -> Dict[str, Any]:
        """
        ...
        for metric in ["avg_precision_at_k", "avg_recall_at_k", "avg_ndcg_at_k"]:
            ...
            for k in k_values:
                ...
                if dense_score > 0:
                    sparse_improvement = ((sparse_score - dense_score) / dense_score) * 100
                    hybrid_improvement = ((hybrid_score - dense_score) / dense_score) * 100
                    
                    comparison["improvement_analysis"][metric][key] = {
                        "sparse_vs_dense": f"{sparse_improvement:+.2f}%",
                        "hybrid_vs_dense": f"{hybrid_improvement:+.2f}%",
                        "hybrid_vs_sparse": f"{((hybrid_score - sparse_score) / sparse_score * 100):+.2f}%" if sparse_score > 0 else "N/A"
                    }
                else:
                    comparison["improvement_analysis"][metric][key] = {
                        "sparse_vs_dense": "N/A",
                ...
                comparison["ranking"][metric][key] = [method for method, score in ranked]
        ...
        if dense_mrr > 0:
            ...
            comparison["improvement_analysis"]["mean_mrr"] = {
                ...
                "hybrid_vs_sparse": f"{((hybrid_mrr - sparse_mrr) / sparse_mrr * 100):+.2f}%" if sparse_mrr > 0 else "N/A"
            }
        else:
            comparison["improvement_analysis"]["mean_mrr"] = {
                "sparse_vs_dense": "N/A",
        ...
        # Rank MRR
        mrr_scores = [("dense", dense_mrr), ("sparse", sparse_mrr), ("hybrid", hybrid_mrr)]
    ...
    def run_full_evaluation(self, collection_name: str = "evaluation_collection") -> Dict[str, Any]:
        """
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        if dense_summary:
            print(f"\nDENSE SEARCH RESULTS:")
            print(f"  Precision@5: {dense_summary.get('avg_precision_at_k', {}).get('@5', 0):.3f}")
            print(f"  Recall@5: {dense_summary.get('avg_recall_at_k', {}).get('@5', 0):.3f}")
            print(f"  nDCG@5: {dense_summary.get('avg_ndcg_at_k', {}).get('@5', 0):.3f}")
        ...
        if sparse_summary:
            print(f"\nSPARSE SEARCH RESULTS:")
            print(f"  Precision@5: {sparse_summary.get('avg_precision_at_k', {}).get('@5', 0):.3f}")
            print(f"  Recall@5: {sparse_summary.get('avg_recall_at_k', {}).get('@5', 0):.3f}")
            print(f"  nDCG@5: {sparse_summary.get('avg_ndcg_at_k', {}).get('@5', 0):.3f}")
        ...
        if hybrid_summary:
            print(f"\nHYBRID SEARCH RESULTS:")
            print(f"  Precision@5: {hybrid_summary.get('avg_precision_at_k', {}).get('@5', 0):.3f}")
            print(f"  Recall@5: {hybrid_summary.get('avg_recall_at_k', {}).get('@5', 0):.3f}")
            print(f"  nDCG@5: {hybrid_summary.get('avg_ndcg_at_k', {}).get('@5', 0):.3f}")
        ...
        if improvements and rankings:
            ...
            print(f"    Precision@5: {improvements.get('avg_precision_at_k', {}).get('@5', {}).get('sparse_vs_dense', 'N/A')}")
            print(f"    Recall@5: {improvements.get('avg_recall_at_k', {}).get('@5', {}).get('sparse_vs_dense', 'N/A')}")
            print(f"    nDCG@5: {improvements.get('avg_ndcg_at_k', {}).get('@5', {}).get('sparse_vs_dense', 'N/A')}")
            print(f"    MRR: {improvements.get('mean_mrr', {}).get('sparse_vs_dense', 'N/A')}")
            
            print(f"  Hybrid Search:")
            print(f"    Precision@5: {improvements.get('avg_precision_at_k', {}).get('@5', {}).get('hybrid_vs_dense', 'N/A')}")
            print(f"    Recall@5: {improvements.get('avg_recall_at_k', {}).get('@5', {}).get('hybrid_vs_dense', 'N/A')}")
            print(f"    nDCG@5: {improvements.get('avg_ndcg_at_k', {}).get('@5', {}).get('hybrid_vs_dense', 'N/A')}")
            print(f"    MRR: {improvements.get('mean_mrr', {}).get('hybrid_vs_dense', 'N/A')}")
            
            print(f"\nRANKINGS (Best to Worst):")
            print(f"  Precision@5: {' > '.join(rankings.get('avg_precision_at_k', {}).get('@5', []))}")
            print(f"  Recall@5: {' > '.join(rankings.get('avg_recall_at_k', {}).get('@5', []))}")
            print(f"  nDCG@5: {' > '.join(rankings.get('avg_ndcg_at_k', {}).get('@5', []))}")
    ...
    def debug_sparse_search_availability(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.rag_service, 'hybrid_engine') and self.rag_service.hybrid_engine:
                ...
                hybrid_bm25_indices = getattr(self.rag_service.hybrid_engine, 'bm25_indices', {})
                debug_info["available_indices"].extend([f"hybrid:{k}" for k in hybrid_bm25_indices.keys()])
                ...
                if collection_name in hybrid_bm25_indices:
                    ...
                    try:
                        test_results = self.rag_service.hybrid_engine._get_sparse_results(
                            "machine learning", collection_name, k=3
                        )
                        if test_results:
                            debug_info["test_search_successful"] = True
                            debug_info["test_results_count"] = len(test_results)
                            logger.info(f"Hybrid BM25 test search successful: {len(test_results)} results")
                        else:
                            logger.warning("Hybrid BM25 test search returned empty results")
                    except Exception as e:
                        debug_info["error_messages"].append(f"Hybrid BM25 test failed: {e}")
            ...
            bm25_indices = getattr(self.rag_service, 'bm25_indices', {})
            debug_info["available_indices"].extend([f"direct:{k}" for k in bm25_indices.keys()])
            ...
            if collection_name in bm25_indices:
                ...
                try:
                    ...
                    if test_results:
                        debug_info["test_search_successful"] = True
                        debug_info["test_results_count"] = len(test_results)
                        logger.info(f"Direct BM25 test search successful: {len(test_results)} results")
                    else:
    ...
    def test_sparse_search_simple(self, collection_name: str) -> bool:
        ...
        try:
            ...
            if result.get("success", False):
                ...
                logger.warning(f"Sparse search test failed: {result.get('message', 'Unknown error')}")
                return False
