...
class MilvusVectorStore:
    ...
    def _embed_batch(self, doc_batch: List) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Embedded batch of {len(doc_batch)} docs in {embedding_time:.2f}s ({len(doc_batch)/embedding_time:.2f} docs/sec)"
            )
        except Exception as e:
    ...
    def add(self, documents, collection_name):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        if (
                            batch_idx % 10 == 0 or batch_idx == num_batches - 1
                        ):  # Log less frequently
                            logger.info(
                                f"Progress: {completed}/{total_docs} documents ({batch_idx+1}/{num_batches} batches)"
                            )
                    except Exception as e:
                        embedding_errors += 1
            ...
            if embedding_errors > 0:
                logger.warning(
                    f"{embedding_errors}/{num_batches} batches failed during embedding"
                )
        except Exception as e:
        ...
        try:
            if not self.client.has_collection(collection_name):
                ...
                schema = [
                    ...
                    FieldSchema(
                        name="vector", dtype=DataType.FLOAT_VECTOR, dim=self.embed_dim
                    ),
                    FieldSchema(name="text", dtype=DataType.VARCHAR, max_length=65535),
                    FieldSchema(
                        name="metadata", dtype=DataType.VARCHAR, max_length=32768
                    ),
                ]
                ...
                try:
                    ...
                    logger.info(
                        f"Successfully created collection {collection_name} with schema-based method"
                    )
                except Exception as e:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Successfully created collection {collection_name} with legacy method"
                        )
                    except Exception as e2:
                        logger.error(
                            f"Both collection creation methods failed for {collection_name}: {e2}"
                        )
                        return False
            else:
                try:
                    ...
                    if "index_params" in str(collection_info):
                        logger.info(
                            f"Collection {collection_name} already exists. Info: {collection_info}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Could not get collection info for {collection_name}: {e}"
                    )
        except Exception as e:
        ...
        for i, batch in enumerate(insert_batches):
            try:
                ...
                if i % 5 == 0 or i == len(insert_batches) - 1:
                    logger.info(
                        f"Inserted batch {i+1}/{len(insert_batches)} ({total_inserted}/{len(all_processed_docs)} docs)"
                    )
            except Exception as e:
                insertion_errors += 1
                ...
                if i == 0 and len(batch) > 0:
                    logger.error(
                        f"First doc schema: {list(batch[0].keys())}, vector len: {len(batch[0]['vector'])}"
                    )
        ...
        if total_inserted == len(all_processed_docs) and insertion_errors == 0:
            ...
            logger.error(
                f"Not all documents were inserted successfully. Inserted: {total_inserted}/{len(all_processed_docs)}, Errors: {insertion_errors}"
            )
            return False
    ...
    def _rerank_results(
        self, query: str, results: List[Dict], top_k: int = 3
    ) -> List[Dict]:
        ...
        try:
            ...
            if hybrid_search and processed_results:
                bm25_results = self._bm25_search(
                    query, processed_results, top_k=initial_limit
                )

                # Combine scores (simple average for now)
                for doc in bm25_results:
                    doc["combined_score"] = (
                        doc["vector_score"] + doc["bm25_score"]
                    ) / 2
            ...
            if rerank and processed_results:
                processed_results = self._rerank_results(
                    query, processed_results, top_k=limit
                )
            else:
                processed_results = processed_results[:limit]

            logger.info(
                f"Query returned {len(processed_results)} results in {time.time() - start_time:.2f}s"
            )
