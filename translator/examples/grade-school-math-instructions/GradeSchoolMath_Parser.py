#!/usr/bin/env python3
"""
Grade School Math Instructions VinAI Parser
This script demonstrates how to parse and translate the qwedsacf/grade-school-math-instructions dataset
...
PARSER_NAME = "GradeSchoolMath"


class GradeSchoolMath(DataParser):
    """
    Grade School Math DataParser using VinAI provider for high-quality English to Vietnamese translation.
    This parser processes the qwedsacf/grade-school-math-instructions dataset and translates it using the
    ...
    def __init__(
        ...
        """
        Initialize the Grade School Math VinAI parser.
    ...
    def read(self) -> None:
        """
        Read the qwedsacf/grade-school-math-instructions dataset from HuggingFace.
        """
        # Call parent read function for validation
        super(GradeSchoolMath, self).read()

        try:
            # Load the dataset
            self.data_read = load_dataset("qwedsacf/grade-school-math-instructions")
    ...
    def convert(self) -> None:
        ...
        super(GradeSchoolMath, self).convert()
        ...
        for split in self.data_read:
            ...
            for data in tqdm(split_data, desc=f"Converting {split} data"):
                ...
                # Use question as question text
                question = data["question"].strip() if data["question"] else ""
                data_dict["question_text"] = question

                # Set answer text from answer
                data_dict["orig_answer_texts"] = (
                    data["answer"] if data["answer"] else ""
                )
...
if __name__ == "__main__":
    ...
    grade_school_math_parser = GradeSchoolMath(
    ...
    grade_school_math_parser.read()
    grade_school_math_parser.convert()
    grade_school_math_parser.save
